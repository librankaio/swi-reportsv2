    ...
    public function index(Request $request)
    {
        if (isset($request->dtfrom)) {
            if ($request->searchtext == null) {
                ...
                // dd($compcode);
                $results = DB::select('CALL rptmutasibahanbaku (?,?,?,?)', [$datefrForm, $datetoForm, $compcode, $lokasi]);

                // $query = DB::select('EXEC rptTest ?,?,?',[$datefrForm,$datetoForm,'BC 4.0']);

                $page = request('page', 1);
                $pageSize = 25;
                // $query = DB::select('CALL rptmutasibahanbaku (?,?,?,?)', [$datefrForm, $datetoForm, $compcode, $lokasi]);
